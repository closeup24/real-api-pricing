...
Этот форк добавляет пункт **«AA + подписки»** в переключатель рядом с AA Intelligence, Terminal-Bench и Code Arena. Он использует состав токенов каждой модели и effort из Artificial Analysis, известные ставки списания и практические замеры подписок. Для каждого тарифа показан метод: квота × ставки, API-калибровка или условный перенос наблюдённого токенного объёма. График использует тот же компонент, цвета, логотипы, масштабирование и выделение выгодных вариантов, что исходный сайт. Ниже расположена таблица; нажатие на точку или строку открывает подробный расчёт.

Запуск из `web`: `npm ci`, затем `npm run dev -- --host 127.0.0.1`. Раздел доступен по `/#board=aa_combined`; старые ссылки `/?view=aa` также работают. Проверки: `npm test`; сборка: `npm run build`. Предыдущие методы сравнения и оценка API доступны по ссылке внутри нового раздела.
